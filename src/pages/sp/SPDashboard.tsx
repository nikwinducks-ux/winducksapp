...
export default function SPDashboard() {
  ...
  const assignedJobs = jobs.filter((j) => j.assignedSpId === currentSpId && j.status === "assigned");
  const pendingOffers = jobs.filter((j) => j.status === "pending").slice(0, 3);
        ...
        <div className="metric-card">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 score-bar-track">
              <div className="score-bar" style={{ width: `${sp.reliabilityScore}%` }} />
            </div>
          </div>
        </div>
        <div className="metric-card">
